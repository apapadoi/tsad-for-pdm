...
def execute(method_names_to_run, MAX_RUNS=100, MAX_JOBS=16, INITIAL_RANDOM=4):
    print("script: edp/run_unsupervised.py")
    ...
    param_space_dict_per_method = [
        {
            ...
            'window': [500, 700, 800, 900, 1000, 1250, 1500, 1750, 1900, 2000, 2250, 2500, 2800],
            'slide': [0.33, 0.5, 1.0],
        ...
        {
            'pattern_length': [25, 50, 75, 100, 200],
            'subsequence_length_multiplier': [3, 4, 5], #4*4 this is the sub size
            'alpha': [0.5, 0.75, 0.25],
            'init_length': [500, 700, 800, 900, 1000, 1250, 1500, 1750, 1900, 2000, 2250, 2500, 2800],
            'batch_size': [500, 700, 800, 900, 1000, 1250, 1500, 1750, 1900, 2000, 2250, 2500, 2800],
            'k': [4, 6, 7, 8, 9, 10],
            'aggregation_strategy': ['avg', 'max']
        }
    ]
    ...
    if 'Chronos' in conda_env:
        param_space_dict_per_method.append(
            {
                'context_length': [500, 700, 800, 900, 1000, 1250, 1500, 1750, 1900, 2000, 2250, 2500, 2800],
        ...
        param_space_dict_per_method.append({
            'context_length': [2800],
            'target': [dataset['target_data'][0].columns[1]],
        })
