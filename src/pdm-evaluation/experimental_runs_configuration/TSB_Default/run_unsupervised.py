...
def execute(method_names_to_run, MAX_RUNS=100, MAX_JOBS=16, INITIAL_RANDOM=4, setup_1_predict=100, setup_1_early=10, max_sizes=None):
    ...
    for dataset_name in dataset_names_to_run:
        ...
        dataset = loadDataset.get_dataset(dataset_name, setup_1_predict=setup_1_predict, setup_1_early=setup_1_early, max_sizes=max_sizes)
        ...
        experiment_names = [
            f"TSB Default Unsupervised {dataset_name}",
        ]
        ...
        param_space_dict_per_method = [ automatic_parameter_generation.default_TSB_unsupervised(method_name,dataset["max_wait_time"]) for method_name in method_names]
