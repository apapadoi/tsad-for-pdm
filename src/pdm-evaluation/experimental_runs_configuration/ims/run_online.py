...
def execute(method_names_to_run, MAX_RUNS=100, MAX_JOBS=16, INITIAL_RANDOM=4):
    print("script: ims/run_online.py")
    ...
    if 'TimeLLM' in conda_env:
        ...
        param_space_dict_per_method.append({
            ...
            'domain_prompt_content': ['Three bearings were installed on a shaft, where a radial load of 6000 lbs is applied by a spring mechanism, and accelerometers were installed on the bearing housing.'],
            'epochs': [25] # 10
        })
    ...
    for current_method, current_method_param_space, current_method_name in zip(methods, param_space_dict_per_method, method_names):
        ...
        for experiment, experiment_name in zip(experiments, experiment_names):
            ...
            current_param_space_dict['profile_size'] = [50, 60, 70, 80, 90, 100, 110, 120, 130, 140, 150, 160, 170, 180, 190, 200] if 'TimeLLM' not in conda_env else [200]

            if current_method_name == 'TIMEMIXERPP':
                current_param_space_dict['profile_size'] = [200]
