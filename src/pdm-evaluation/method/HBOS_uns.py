...
class HBOSUns(UnsupervisedMethodInterface):
    def __init__(self, event_preferences: EventPreferences,window,n_bins=10, alpha=0.1, tol=0.5, sub_sequence_length=1, *args, **kwargs):
        ...
        self.window = window
        self.semi_model=HBOS(event_preferences=event_preferences,n_bins=self.n_bins, alpha=self.alpha, tol=self.tol, sub_sequence_length=self.sub_sequence_length)
