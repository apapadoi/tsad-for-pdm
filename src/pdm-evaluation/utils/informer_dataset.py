...
class InformerDataset:
    ...
    def _get_borders(self, df):
        # if self.data_split != 'test':
        n_vali = self.vali_points
        n_train = self.train_points - self.vali_points
        #     n_test = 0
        # else:
        #     n_vali = 0
        #     n_train = 0
        n_test = len(df) - n_vali - n_train
        assert n_vali + n_train + n_test == len(df)
        train_end = n_train
        val_end = n_train + n_vali
        test_start = val_end - self.seq_len
        test_end = test_start + n_test + self.seq_len

        train = slice(0, train_end)
        val = slice(train_end + 1, val_end)
    ...
    def _read_data(self, df):
        ...
        data_splits = self._get_borders(df)
