...
duration_stats_df = df.groupby(['Technique', 'Flavor', 'Dataset'])['Duration'].agg(['min', 'median', 'max']).reset_index()
...
for i, (flavor, flavor_name) in enumerate(zip(flavors_list, flavor_names)):
    ...
    flavor_data = duration_stats_df[duration_stats_df['Flavor'] == flavor]
    ...
    table = summary_df.pivot(index='Dataset', columns='Technique', values='Duration')
    ...
    try:
        # Perform autorank analysis (ascending order since lower duration is better)
        result = autorank(
            table, 
            alpha=0.05,
            verbose=False,
            order='ascending',  # Lower duration is better
...
plt.tight_layout()
plt.savefig('cd_diagrams_by_flavor_duration.pdf', format='pdf', bbox_inches='tight')
plt.savefig('cd_diagrams_by_flavor_duration.png', format='png', bbox_inches='tight', dpi=300)
plt.show()

print("Critical difference diagrams saved as 'cd_diagrams_by_flavor_duration.pdf' and 'cd_diagrams_by_flavor_duration.png'")
