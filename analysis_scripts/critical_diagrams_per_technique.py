...
# Create summary with max values per technique-flavor combination
summary = df.groupby(['Dataset', 'Technique', 'Flavor'])[METRIC].max().reset_index()
...
for technique in all_techniques:
    ...
    try:
        ...
        best_row = result.rankdf.iloc[-1]  # Last row has the lowest mean rank (best)
        best_flavor = result.rankdf.index[-1]
        
        print(f"  Best flavor: {best_flavor}")
        print(f"  Mean rank: {best_row['meanrank']:.4f}")
        # print(f"  Full ranking results:")
        # print(result.rankdf)
